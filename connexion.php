...
if($_POST){
	...
	if($resultat -> rowCount() > 0){ // Si $resultat -> rowCount() est sup�rieur � 0 cela signifie que l'utilisateur existe bien dans la BDD
		...
		if(md5($_POST['mdp']) == $membre['mdp']){ // Les deux MDP sont semblables je peux connecter l'utilisateur
			// Je fais une boucle qui va r�cup�rer toutes les infos du membre pour les enregistrer dans la SESSION. J'organise ma SESSION en array Multidimentionnel (membre, panier)
